...
const ScreenZero = ({ navigation, route }) => {
  ...
  const validation = () => {
    if (age == null || height == null || weight == null) {
      ...
      if (age > 70 || age < 12) alert("Please enter age between 15-65");
      else if (height > 210 || height < 120)
        alert("Please enter height between 150-200 Cm");
      else if (weight > 150 || weight < 40)
        alert("Please enter weight between 40-120 Kg");
      else
  ...
  return (
    <DismissKeyboard>
      ...
      >
        ...
        <View style={{ justifyContent: "center" }}>
          <View style={styles.view1}>
            <Text style={styles.t2}>Height</Text>
        ...
        <View style={{ justifyContent: "center" }}>
          <View style={styles.view1}>
            <Text style={styles.t2}>Weight</Text>
...
const ScreenOne = ({ navigation, route }) => {
  ...
  const heightf = route.params.height;
...
const ScreenTwo = ({ navigation, route }) => {
  ...
  const heightf = route.params.heightf;
...
const ScreenFive = ({ navigation, route }) => {
  ...
  const connect = () => {
    fetch("http://192.168.1.110:4008/deleteFORreset", {
      method: "DELETE",
      headers: {
      ...
      .then((res) => {
        if (res.success === true) {
          fetch("http://192.168.1.110:4008/reset", {
            method: "PUT",
            headers: {
              Accept: "application/json",
              "Content-Type": "application/json",
...
const Reset = ({ navigation, route }) => {
  const { user, gender, firstname, lastname } = route.params;
  ...
  return (
    <Stack.Navigator>
      <Stack.Screen
        ...
        initialParams={{ user: user, gender: gender }}
